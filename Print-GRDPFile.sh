...
if [[ -z "$USER_CONFIG_FILE" ]]; then
    USER_CONFIG_FILE="$SCRIPT_DIR/userconfig.json"
fi
...
update_check() {
    ...
    if [[ "$use_prerelease" == "true" ]]; then
        ...
        # Get all releases
        local all_releases=$(curl -s "${release_api_url/\/latest/}")
        local latest_release=$(echo "$all_releases" | jq -r '.[0]')
    else
        echo "Checking for latest stable release only..."
        local latest_release=$(curl -s "$release_api_url")
    fi
    
    local release_version=$(echo "$latest_release" | jq -r '.tag_name' | sed 's/^v//')
    ...
    if [[ "$(printf '%s\n' "$release_version" "$current_version" | sort -V | tail -n1)" != "$current_version" ]]; then
        ...
        local download_url=$(echo "$latest_release" | jq -r '.zipball_url')
...
main() {
    ...
    if [[ "${INPUT_FILE,,}" == *.grdp ]]; then
        ...
        for ((i=0; i<entries_count; i++)); do
            local filename=$(jq -r ".[$i].Filename" "$settings_file")
            ...
            local file_path="$temp_folder/$filename"
            
            if [[ ! -f "$file_path" ]]; then
                echo "Warning: File $filename not found in archive, skipping."
                continue
            fi
            ...
            if [[ "${file_path,,}" == *.pdf ]]; then
                ...
                # Open non-PDF file with default application
                local downloads_folder="$HOME/Downloads"
                local unique_filepath=$(get_unique_filename "$downloads_folder/$(basename "$file_path")")
